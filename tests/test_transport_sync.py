...
def test_socks5_proxy_ipv4(url: str, rdns: bool, target_ssl_ca: trustme.CA) -> None:
...
def test_socks5_proxy_ipv6(url: str, target_ssl_ca: trustme.CA) -> None:
    transport = SyncProxyTransport.from_url(
        SOCKS5_IPV6_URL,
        verify=create_ssl_context(url, ca=target_ssl_ca),
    )
...
@pytest.mark.parametrize("url", (TEST_URL_IPV4, TEST_URL_IPV4_HTTPS))
@pytest.mark.parametrize("rdns", (True, False))
def test_socks4_proxy(url: str, rdns: bool, target_ssl_ca: trustme.CA) -> None:
    transport = SyncProxyTransport.from_url(
        SOCKS4_URL,
        rdns=rdns,
        verify=create_ssl_context(url, ca=target_ssl_ca),
...
@pytest.mark.parametrize("url", (TEST_URL_IPV4, TEST_URL_IPV4_HTTPS))
def test_http_proxy(url: str, target_ssl_ca: trustme.CA) -> None:
    transport = SyncProxyTransport.from_url(
        HTTP_PROXY_URL,
        verify=create_ssl_context(url, ca=target_ssl_ca),
    )
...
def test_proxy_http2(target_ssl_ca: trustme.CA) -> None:
...
def test_failed_proxy_connection() -> None:
    url = TEST_URL_IPV4_HTTPS
    proxy_url = HTTP_PROXY_URL
